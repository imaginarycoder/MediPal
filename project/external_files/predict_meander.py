...
def openFile_svm():
    with open(os.path.join(EXTERNAL_DIR, 'model_parkinson_svm_meander.pkl'), 'rb') as file:
        model = pickle.load(file)
        file.close()
    return model

def openFile_naive_bayes():
    with open(os.path.join(EXTERNAL_DIR, 'model_parkinson_bayes_meander.pkl'), 'rb') as file:
        model = pickle.load(file)
        file.close()
    return model
